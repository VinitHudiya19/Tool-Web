...
import { getRelatedSeoTools, getSeoTool } from "@/lib/seo-tools/tools.config";
import {
...
/**
 * The universal SEO tool page.
 *
 * Fixes the section order required by BUILD_STANDARDS.md and emits the full
 * schema set, so individual tools supply only their interactive part. This is a
 * server component: everything except the tool itself is static HTML, keeping
 * the supporting content out of the JavaScript bundle.
 */
export default function SeoToolPage({
  ...
  const related = getRelatedSeoTools(slug);
  ...
  return (
    <>
      ...
      {/* Amber is the SEO category accent from DESIGN_SYSTEM.md. */}
      <div className="flex min-h-screen flex-col" style={getCategoryThemeVars("amber")}>
        <Header />
        ...
        <main className="mx-auto w-full max-w-[1200px] flex-grow px-4 py-8 sm:px-6 lg:px-8">
          <nav aria-label="Breadcrumb" className="mb-6">
            <ol className="flex flex-wrap items-center gap-1.5 text-xs font-medium text-text-2">
              ...
              <li>
                <Link href="/seo-tools" className="transition-colors hover:text-primary">
                  SEO Tools
                </Link>
              </li>
              <ChevronRight size={12} className="opacity-40" aria-hidden="true" />
          ...
          {/* The tool itself, above the supporting content */}
          ...
          />
          <TakeawaysSection
            title={`${tool.name} in short`}
            takeaways={tool.keyTakeaways}
          />
          <FaqSection title={`${tool.name} questions`} faqs={tool.faqs} />

          {related.length > 0 && (
            <section className="border-t border-border-custom py-14">
              <SectionHeading eyebrow="Discover" title="Related SEO tools" />
