...
import { getImageTool, getRelatedImageTools } from "@/lib/image/tools.config";
import {
...
export default function ImageToolPage({
  ...
  const related = getRelatedImageTools(slug);
  const path = `/image-tools/${tool.slug}`;
  ...
  return (
    <>
      ...
      {/* Pink is the image category accent from DESIGN_SYSTEM.md. */}
      <div className="flex min-h-screen flex-col" style={getCategoryThemeVars("pink")}>
        <Header />
        ...
        <main className="mx-auto w-full max-w-[1200px] flex-grow px-4 py-8 sm:px-6 lg:px-8">
          ...
          <header className="mb-8">
            ...
            <p className="mt-4 inline-flex items-start gap-2 rounded-lg border border-border-custom bg-surface px-3 py-2 text-xs font-medium text-text-2">
              ...
              <span>Runs entirely in your browser — your images are never uploaded.</span>
            </p>
          </header>
          ...
          {related.length > 0 && (
            <section className="border-t border-border-custom py-14">
              <SectionHeading eyebrow="Discover" title="Related image tools" />
