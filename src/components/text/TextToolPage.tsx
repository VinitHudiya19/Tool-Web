...
import { getRelatedTextTools, getTextTool } from "@/lib/text/tools.config";
import {
...
export default function TextToolPage({
  ...
  const related = getRelatedTextTools(slug);
  const path = `/text-tools/${tool.slug}`;
  ...
  return (
    <>
      ...
      {/* Blue is the text category accent from the category config. */}
        ...
        <main className="mx-auto w-full max-w-[1200px] flex-grow px-4 py-8 sm:px-6 lg:px-8">
          <nav aria-label="Breadcrumb" className="mb-6">
            <ol className="flex flex-wrap items-center gap-1.5 text-xs font-medium text-text-2">
              ...
              <li>
                <Link href="/text-tools" className="transition-colors hover:text-primary">
                  Text Tools
                </Link>
              </li>
              <ChevronRight size={12} className="opacity-40" aria-hidden="true" />
          ...
          <header className="mb-8">
            ...
            <p className="mt-4 inline-flex items-start gap-2 rounded-lg border border-border-custom bg-surface px-3 py-2 text-xs font-medium text-text-2">
              ...
              <span>Runs entirely in your browser — your text is never uploaded.</span>
            </p>
          </header>
          ...
          {related.length > 0 && (
            <section className="border-t border-border-custom py-14">
              <SectionHeading eyebrow="Discover" title="Related text tools" />
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {related.map((item) => (
                  <Link
                    key={item.slug}
                    href={`/text-tools/${item.slug}`}
