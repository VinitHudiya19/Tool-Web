...
import {
  getBusinessTool,
  getRelatedBusinessTools,
} from "@/lib/business/tools.config";
import {
...
/**
 * The universal business tool page.
 *
 * A server component: only the document builder ships as client JavaScript,
 * so the supporting content is static HTML that crawlers read without
 * executing anything.
 */
export default function BusinessToolPage({
  ...
  const related = getRelatedBusinessTools(slug);
  const path = `/business-tools/${tool.slug}`;
  ...
  return (
    <>
      ...
      {/* Teal is the finance/business accent from DESIGN_SYSTEM.md. */}
      <div className="flex min-h-screen flex-col" style={getCategoryThemeVars("teal")}>
        ...
        <main className="mx-auto w-full max-w-[1200px] flex-grow px-4 py-8 sm:px-6 lg:px-8">
          ...
          <header className="mb-8 print:hidden">
            ...
            <p className="mt-4 inline-flex items-start gap-2 rounded-lg border border-border-custom bg-surface px-3 py-2 text-xs font-medium text-text-2">
              ...
              <span>
                Runs entirely in your browser — client details and amounts are never
                uploaded.
              </span>
            </p>
          </header>
          ...
          <div className="print:hidden">
            ...
            {related.length > 0 && (
              <section className="border-t border-border-custom py-14">
                <SectionHeading eyebrow="Discover" title="Related business tools" />
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  {related.map((item) => (
                    <Link
                      key={item.slug}
                      href={`/business-tools/${item.slug}`}
